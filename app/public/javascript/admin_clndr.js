    ...
    for (let d = 0; d < 7; d++) {
      if (w == 0 && d < startDay) {
        ...
        if (comeFlg == targetDay) {
          calendarHtml += `<td class="calendar_td" id="come" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        } else if (delayFlg == targetDay) {
          calendarHtml += `<td class="calendar_td" id="delay" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        } else if (absenceFlg == targetDay) {
          calendarHtml += `<td class="calendar_td" id="absence" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        } else {
          calendarHtml += `<td class="calendar_td" id="other" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        }
      }
    }

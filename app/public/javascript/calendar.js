...
function createCalendar(year, month, userData) {
  ...
  for (let i = 0; i < weeks.length; i++) {
    calendarHtml += '<td class="user-calendar-td">' + weeks[i] + '</td>';
  }
  for (let w = 0; w < 6; w++) {
    ...
    for (let d = 0; d < 7; d++) {
      if (w == 0 && d < startDay) {
        ...
        calendarHtml += '<td class="is-disabled user-calendar-td">' + num + '</td>';
        ...
        if (comeFlg == targetDay) {
          calendarHtml += `<td class="user-calendar-td" id="come" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        } else if (delayFlg == targetDay) {
          calendarHtml += `<td class=" user-calendar-td" id="delay" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        } else if (absenceFlg == targetDay) {
          calendarHtml += `<td class="user-calendar-td" id="absence" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        } else {
          calendarHtml += `<td class="user-calendar-td" data-date="${year}/${month}/${dayCount}">${dayCount}</td>`;
          dayCount++;
        }
      }
    }
